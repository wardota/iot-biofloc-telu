...
//   // Update the charts with the new range
//   // Get the latest readings and plot them on charts (the number of plotted readings corresponds to the chartRange value)
//   dbRef.orderByKey().limitToLast(chartRange).once('value', (snapshot) => {
//     const readings = snapshot.val();

//     // Batch the readings into smaller chunks
...
//       Promise.all(
//         batch.map((reading) => {
//           const { temperature, humidity, pressure, timestamp } = reading;
//            console.log('cek bo1y');
//           return Promise.resolve({ temperature, humidity, pressure, timestamp });
//         })
//       ).then((results) => {
        
//         console.log('cek boyad');
//         results.forEach((result) => {
//           plotValues(chartT, result.timestamp, result.temperature);
//           plotValues(chartH, result.timestamp, result.humidity);
//           plotValues(chartP, result.timestamp, result.pressure);
//         });
//       });
//     });
//   });
